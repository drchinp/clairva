"""
File: app/agents/A0002_reporting_agent/agent.py

Purpose:
Reporting & Analysis Agent responsible for
data summarization and KPI extraction.

Enhancements:
- PromptGuard protection
- Agent Memory retrieval
- Agent Memory storage
"""

from app.agents.base_agent import BaseAgent
from app.agents.A0002_reporting_agent.schema import ReportingAgentInput
from app.agents.A0002_reporting_agent.tools import summarize_data, extract_kpis

from app.mcp.tool_planner import plan_tool_usage
from app.mcp.tool_executor import execute_tool

from app.security.prompt_guard import validate_prompt
from app.memory.agent_memory import store_memory, retrieve_memory

from app.utils.logger import get_logger

logger = get_logger(__name__)


class ReportingAgent(BaseAgent):

    def __init__(self):

        super().__init__(
            agent_id="A0002",
            agent_name="Reporting & Analysis Agent"
        )

    def run(self, input_data: dict):

        self.log_start(input_data)

        validated_input = ReportingAgentInput(**input_data)

        data = validated_input.data

        # ---------------------------------
        # PromptGuard Protection
        # ---------------------------------

        data = validate_prompt(data)

        # ---------------------------------
        # Retrieve Agent Memory
        # ---------------------------------

        try:

            memory_context = retrieve_memory(
                self.agent_id,
                data
            )

            logger.info(
                f"A0002 retrieved memory items: {len(memory_context)}"
            )

        except Exception as e:

            logger.warning(
                f"Memory retrieval failed: {e}"
            )

            memory_context = []

        # -------------------------
        # MCP Tool Planning
        # -------------------------

        tool_plan = plan_tool_usage(
            query=data,
            agent_id=self.agent_id
        )

        summary = None
        kpis = None
        tool_used = None

        if tool_plan.get("tool"):

            tool_used = tool_plan["tool"]

            logger.info(f"A0002 selected tool: {tool_used}")

            result = execute_tool(
                self.agent_id,
                tool_used,
                tool_plan.get("arguments", {})
            )

            if isinstance(result, dict):

                summary = result.get("summary")
                kpis = result.get("kpis")

        # -------------------------
        # Fallback Tools
        # -------------------------

        if not summary:

            summary = summarize_data(data)

        if not kpis:

            kpis = extract_kpis(data)

        # ---------------------------------
        # Store Agent Memory
        # ---------------------------------

        try:

            memory_text = f"""
Summary:
{summary}

KPIs:
{kpis}
"""

            store_memory(
                self.agent_id,
                memory_text
            )

            logger.info("A0002 memory stored")

        except Exception as e:

            logger.warning(
                f"Memory store failed: {e}"
            )

        result = {
            "status": "success",
            "summary": summary,
            "kpis": kpis,
            "tool_used": tool_used,
            "memory_used": len(memory_context)
        }

        self.log_finish(result)

        return result